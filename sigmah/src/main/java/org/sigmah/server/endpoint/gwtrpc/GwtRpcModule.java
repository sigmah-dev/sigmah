...
import org.sigmah.server.endpoint.export.sigmah.SigmahImportServlet;
...
public class GwtRpcModule extends ServletModule {
	...
	protected void configureServlets() {
		...
		serve("/Sigmah/download").with(FileDownloadServlet.class); // Files
																   // download.
		serve("/Sigmah/backup").with(FilesBackupServlet.class);
		serve("/Sigmah/management").with(ManagementServlet.class);
		serve("/Sigmah/image-provider").with(ImageServlet.class); // Image
																  // provider.
		serve("/Sigmah/password").with(PasswordManagementServlet.class); // Password
																		 // lost
																		 // service.
		serve("/Sigmah/models").with(ExportModelServlet.class); // Model
																// export/import
		serve("/Sigmah/export").with(SigmahExportServlet.class); // Exports.
		serve("/Sigmah/import").with(SigmahImportServlet.class);
	}
}
