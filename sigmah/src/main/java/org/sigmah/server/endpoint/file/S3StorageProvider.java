...
/**
 * Implements a {@code FileStorageProvider} using Amazon AWS's S3 storage web service. 
 * 
 * This provider can be configured from sigmah.properties with the following properties:
 *
 * <pre>
...
public class S3StorageProvider implements FileStorageProvider {
	...
	public S3StorageProvider(Properties config) {
		this.credentials = new BasicAWSCredentials(config.getProperty("aws.access_key"), config.getProperty("aws.secret_key"));
		this.bucketName = config.getProperty("repository.s3_bucket");
	}
