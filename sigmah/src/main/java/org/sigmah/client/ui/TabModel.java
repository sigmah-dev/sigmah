...
public class TabModel {
    ...
    /**
     * Add a new tab to the tab bar. If the tab with the same PageState.pageId already exists, it is updated to reflect
     * the new given values. The closeable state cannot be changed once it has been set.
     * 
     * @param title
     *            Title to display in the tab bar. <code>null</code> displays a loading message.
     * @param state
     *            State to restore when the tab is selected. The PageId value is used as a primary key.
     * @param closeable
     * @return
     */
    ...
    public void remove(Tab t) {
        ...
        boolean removeGranted = fireTabRemoved(index);
        if (removeGranted) {
            tabs.remove(index);
        }
    }
    ...
    protected boolean fireTabRemoved(final int index) {
        boolean removeGranted = true;
        for (Listener listener : listeners) {
            boolean isTabRemoved = listener.tabRemoved(index);
            if (!isTabRemoved) {
                removeGranted = false;
            }
        }
