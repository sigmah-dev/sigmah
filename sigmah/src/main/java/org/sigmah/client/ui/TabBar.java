...
public class TabBar extends Composite {
    ...
    public TabAction getLastAction() {
        return lastAction;
    }
    ...
    private void linkWith(final TabModel model) {
        ...
        model.addListener(new TabModel.Listener() {
            ...
            @Override
            public boolean tabRemoved(int index) {
                ...
                if (index == selectedIndex) {
                    ...
                    if (newIndex != -1) {
                        navigationError = displayTab(model.get(newIndex));
                        if (navigationError == NavigationError.NONE) {
                            removeTab(index);
                            scrollRightButton.setEnabled(leftTabIndex < model.size() - 2);
                ...
                return (navigationError == NavigationError.NONE);
            }
        });
    }
     ...
     *            the Tab to display.
     */
    public NavigationError displayTab(final Tab tab) {
        NavigationEvent event = new NavigationEvent(NavigationHandler.NavigationRequested, tab.getState(), null, this);
        eventBus.fireEvent(event);
        return event.getNavigationError();
    }
    ...
    private void addTab(final Tab tab) {
        ...
        final ClickHandler clickHandler = new ClickHandler() {
            ...
            @Override
            public void onClick(ClickEvent event) {
                NavigationError navigationError = displayTab(tab);
                if (navigationError == NavigationError.NONE) {
                    setSelectedIndex(model.indexOf(tab));
                }
                lastAction = TabAction.NAVIGATE;
            }
        };
