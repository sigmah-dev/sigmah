...
public class ProjectLogFrameGrid {
	 ...
	 */
	private static final String CSS_ADD_ACTION_STYLE_NAME = CSS_LOG_FRAME_GRID_STYLE_NAME
			+ "-add-action";

	/**
	 * CSS style name for the action button which add groups.
	 */
	private static final String CSS_ADD_GROUP_ACTION_STYLE_NAME = CSS_LOG_FRAME_GRID_STYLE_NAME
			+ "-add-group-action";

	/**
	 * CSS style name for the labels which display codes.
	 */
	private static final String CSS_CODE_LABEL_STYLE_NAME = CSS_LOG_FRAME_GRID_STYLE_NAME
			+ "-code-label";

	/**
	 * CSS style name for the labels which display codes (active state).
	 */
	private static final String CSS_CODE_LABEL_ACTIVE_STYLE_NAME = CSS_CODE_LABEL_STYLE_NAME
			+ "-active";

	/**
	 * CSS style name for the menus buttons.
	 */
	private static final String CSS_MENU_BUTTON_STYLE_NAME = CSS_LOG_FRAME_GRID_STYLE_NAME
			+ "-menu-button";

	/**
	 * CSS style name for the menus buttons (active state).
	 */
	private static final String CSS_MENU_BUTTON_ACTIVE_STYLE_NAME = CSS_MENU_BUTTON_STYLE_NAME
			+ "-active";
	...
	protected void initTable() {
		...
		final Label interventionLogicLabel = new Label(
				I18N.CONSTANTS.logFrameInterventionLogic());
		final Label indicatorsLabel = new Label(
				I18N.CONSTANTS.logFrameIndicators());
		final Label risksLabel = new Label(I18N.CONSTANTS.logFrameRisks());
		final Label assumptionsLabel = new Label(
				I18N.CONSTANTS.logFrameAssumptions());
		...
		// Specific objectives.
		final Label specificObjectivesLabel = new Label(
				I18N.CONSTANTS.logFrameSpecificObjectives() + " ("
						+ I18N.CONSTANTS.logFrameSpecificObjectivesCode() + ")");

		final Label specificObjectivesButton = new Label(
				I18N.CONSTANTS.logFrameAddRow());
		...
		final Label specificObjectivesGroupsButton = new Label(
				I18N.CONSTANTS.logFrameAddGroup());
		specificObjectivesGroupsButton
				.addStyleName(CSS_ADD_GROUP_ACTION_STYLE_NAME);
		...
		if (!readOnly) {
			...
			specificObjectivesGrid.setWidget(2, 0,
					specificObjectivesGroupsButton);
		}

		// Expected results.
		final Label exceptedResultsLabel = new Label(
				I18N.CONSTANTS.logFrameExceptedResults() + " ("
						+ I18N.CONSTANTS.logFrameExceptedResultsCode() + ")");

		final Label expectedResultsButton = new Label(
				I18N.CONSTANTS.logFrameAddRow());
		...
		final Label expectedResultsGroupsButton = new Label(
				I18N.CONSTANTS.logFrameAddGroup());
		expectedResultsGroupsButton
				.addStyleName(CSS_ADD_GROUP_ACTION_STYLE_NAME);
		...
		// Activities.
		final Label activitiesLabel = new Label(
				I18N.CONSTANTS.logFrameActivities());

		final Label activitiesButton = new Label(
				I18N.CONSTANTS.logFrameAddRow());
		...
		final Label activitiesGroupsButton = new Label(
				I18N.CONSTANTS.logFrameAddGroup());
		...
		// Prerequisites.
		final Label prerequisitesLabel = new Label(
				I18N.CONSTANTS.logFramePrerequisites());

		final Label prerequisitesButton = new Label(
				I18N.CONSTANTS.logFrameAddRow());
		...
		final Label prerequisitesGroupButton = new Label(
				I18N.CONSTANTS.logFrameAddGroup());
		...
		specificObjectivesView
				.addFlexTableViewListener(new FlexTableViewListener() {
					...
					public void rowRemoved(RowsGroup<?> group, Row<?> row) {
						...
						final Integer max = logFrameModel
								.getSpecificObjectivesMax();
						if (max != null && max > 0
								&& max > specificObjectivesView.getRowsCount()) {
					...
					public void groupAdded(RowsGroup<?> group) {
						...
						final Integer max = logFrameModel
								.getSpecificObjectivesGroupsMax();
						if (max != null
								&& max > 1
								&& max <= specificObjectivesView
										.getGroupsCount()) {
							specificObjectivesGroupsButton.setVisible(false);
						}
					}
		...
		expectedResultsView
				.addFlexTableViewListener(new FlexTableViewListener() {
					...
					public void rowRemoved(RowsGroup<?> group, Row<?> row) {
						...
						// Checks if the max number of elements is reached.
						final Integer max = logFrameModel
								.getExpectedResultsMax();
						if (max != null && max > 0
								&& max > expectedResultsView.getRowsCount()) {
					...
					public void groupAdded(RowsGroup<?> group) {
						...
						if (max != null && max > 1
								&& max <= expectedResultsView.getGroupsCount()) {
							expectedResultsGroupsButton.setVisible(false);
						}
					}
				});
		...
		activitiesView.addFlexTableViewListener(new FlexTableViewListener() {
			...
			public void rowRemoved(RowsGroup<?> group, Row<?> row) {
				...
				if (max != null && max > 0
						&& max > activitiesView.getRowsCount()) {
			...
			public void groupAdded(RowsGroup<?> group) {
				...
				if (max != null && max > 1
						&& max <= activitiesView.getGroupsCount()) {
					activitiesGroupsButton.setVisible(false);
				}
			}
		});
		...
		prerequisitesView.addFlexTableViewListener(new FlexTableViewListener() {
			...
			public void rowRemoved(RowsGroup<?> group, Row<?> row) {
				...
				if (max != null && max > 0
						&& max > prerequisitesView.getRowsCount()) {
			...
			public void groupAdded(RowsGroup<?> group) {
				...
				if (max != null && max > 1
						&& max <= prerequisitesView.getGroupsCount()) {
					prerequisitesGroupButton.setVisible(false);
				}
			}
		});
		...
		expectedResultsPolicy = new CodePolicy<ExpectedResultDTO>() {
			...
			public String getCode(int code, ExpectedResultDTO userObject) {
				...
				if ((parent = userObject.getParentSpecificObjectiveDTO()) != null) {
					sb.append(specificObjectivesPolicy.getCode(
							parent.getCode(), parent));
				}
		...
		activitiesPolicy = new CodePolicy<LogFrameActivityDTO>() {
			...
			public String getCode(int code, LogFrameActivityDTO userObject) {
				...
				if ((parent = userObject.getParentExpectedResultDTO()) != null) {
					sb.append(expectedResultsPolicy.getCode(parent.getCode(),
							parent));
				}
	...
	public void displayLogFrame(LogFrameDTO logFrame, boolean enabled) {
		...
		for (final SpecificObjectiveDTO objective : logFrame
				.getSpecificObjectivesDTONotDeleted()) {
			addSpecificObjective(objective);
		}

		// Displays the prerequisietes.
		for (final PrerequisiteDTO prerequisite : logFrame
				.getPrerequisitesDTONotDeleted()) {
	...
	protected void addSpecificObjectivesGroup() {
		...
		if (!logFrameModel.getEnableSpecificObjectivesGroups()) {
			MessageBox.alert(I18N.CONSTANTS.logFrameUnauthorizedAction(),
					I18N.CONSTANTS.logFrameGroupsDisabledOS(), null);
			return;
		}
		...
		if (max != null && max > 1
				&& max <= specificObjectivesView.getGroupsCount()) {
			MessageBox.alert(I18N.CONSTANTS.logFrameUnauthorizedAction(),
					I18N.CONSTANTS.logFrameGroupsMaxReachedOS(), null);
			return;
		}

		// Asks for the new group label.
		MessageBox.prompt(I18N.CONSTANTS.logFrameAddGroup(),
				I18N.CONSTANTS.logFrameAddGroupToOS(), false,
				new Listener<MessageBoxEvent>() {
					...
					public void handleEvent(MessageBoxEvent be) {
						...
						if (Dialog.OK.equals(be.getButtonClicked().getItemId())) {
							...
							final LogFrameGroupDTO group = logFrame
									.addGroup(
											label,
											LogFrameGroupType.SPECIFIC_OBJECTIVE);
							//Test
							Collection<LogFrameGroupDTO> groups = logFrame.getAllGroups(LogFrameGroupType.SPECIFIC_OBJECTIVE);
							Log.debug("Current SO Group number : " + groups.size()+"\n" );
							for(LogFrameGroupDTO g:groups)
							{
								Log.debug("Group: "+g.getLabel()+"\n");
							}
	 ...
	 */
	private RowsGroup<LogFrameGroupDTO> addSpecificObjectivesGroup(
			final LogFrameGroupDTO group) {

		ensureLogFrame();

		final RowsGroup<LogFrameGroupDTO> g = new RowsGroup<LogFrameGroupDTO>(
				group) {
			...
			public Widget getWidget() {
				...
				if (!readOnly) {
					grid.setWidget(0, 1,
							buildGroupMenu(specificObjectivesView, this, label));
				}
	...
	protected void addSpecificObjective() {
		...
		if (max != null && max > 0
				&& max <= specificObjectivesView.getRowsCount()) {
			MessageBox.alert(I18N.CONSTANTS.logFrameUnauthorizedAction(),
					I18N.CONSTANTS.logFrameMaxReachedOS(), null);
			return;
		}
		...
		if (logFrameModel.getEnableSpecificObjectivesGroups()) {
			...
			formWindow
					.addChoicesList(
							I18N.CONSTANTS.logFrameGroup(),
							logFrame.getAllGroups(LogFrameGroupType.SPECIFIC_OBJECTIVE),
							false, "label");
			formWindow.addFormSubmitListener(new FormSubmitListener() {
				...
				public void formSubmitted(Object... elements) {
					...
					final SpecificObjectiveDTO objective = logFrame
							.addSpecificObjective();
			...
			formWindow.show(I18N.CONSTANTS.logFrameAddOS(),
					I18N.CONSTANTS.logFrameSelectGroupOS());
		}
		...
		// be selected.
			...
			final LogFrameGroupDTO group = logFrame
					.getDefaultGroup(LogFrameGroupType.SPECIFIC_OBJECTIVE);
	 ...
	 */
	private void addSpecificObjective(
			final SpecificObjectiveDTO specificObjective) {

		// Checks if the objective is correct.
		if (specificObjective == null) {
			throw new NullPointerException(
					"specific objective must not be null");
		}

		// Retrieves the group.
		final LogFrameGroupDTO logFrameGroup = specificObjective
				.getLogFrameGroupDTO();
		...
		sb.append(I18N.CONSTANTS.logFrameSpecificObjectivesCode());
		sb.append(" ");
		sb.append(specificObjectivesPolicy.getCode(specificObjective.getCode(),
				specificObjective));
		...
		// Adds the row.
		specificObjectivesView.insertRow(specificObjective.getPosition(),
				logFrameGroup.getClientSideId(), new Row<SpecificObjectiveDTO>(
						specificObjective) {

					@Override
					public boolean isSimilar(int column,
							SpecificObjectiveDTO userObject,
					...
					public Widget getWidgetAt(int column,
							final SpecificObjectiveDTO userObject) {
						...
						case 1:
							...
							if (!readOnly) {
								grid.setWidget(
										0,
										1,
										buildSpecificObjectiveMenu(this,
												codeLabel));
							}
						...
						case 2:
							...
							interventionLogicTextBox
									.addStyleName("html-textbox");
							interventionLogicTextBox.setEnabled(!readOnly);

							if (userObject != null) {
								interventionLogicTextBox.setText(userObject
										.getInterventionLogic());
							}
							...
							interventionLogicTextBox
									.addChangeHandler(new ChangeHandler() {
										...
										public void onChange(ChangeEvent e) {
											userObject
													.setInterventionLogic(interventionLogicTextBox
															.getText());
											fireLogFrameEdited();
										}
									});
						...
						case 5:
							...
							if (userObject != null) {
								assumptionsTextBox.setText(userObject
										.getAssumptions());
							}
							...
							assumptionsTextBox
									.addChangeHandler(new ChangeHandler() {
										...
										public void onChange(ChangeEvent e) {
											userObject
													.setAssumptions(assumptionsTextBox
															.getText());
											fireLogFrameEdited();
										}
									});
		...
		if (specificObjective.getExpectedResultsDTONotDeleted() != null) {
			for (final ExpectedResultDTO result : specificObjective
					.getExpectedResultsDTONotDeleted()) {
	...
	protected void addExpectedResultsGroup() {
		...
		if (!logFrameModel.getEnableExpectedResultsGroups()) {
			MessageBox.alert(I18N.CONSTANTS.logFrameUnauthorizedAction(),
					I18N.CONSTANTS.logFrameGroupsDisabledER(), null);
			return;
		}
		...
		if (max != null && max > 1
				&& max <= expectedResultsView.getGroupsCount()) {
			MessageBox.alert(I18N.CONSTANTS.logFrameUnauthorizedAction(),
					I18N.CONSTANTS.logFrameGroupsMaxReachedER(), null);
			return;
		}

		// Asks for the new group label.
		MessageBox.prompt(I18N.CONSTANTS.logFrameAddGroup(),
				I18N.CONSTANTS.logFrameAddGroupToER(), false,
				new Listener<MessageBoxEvent>() {
					...
					public void handleEvent(MessageBoxEvent be) {
						...
						if (Dialog.OK.equals(be.getButtonClicked().getItemId())) {
							...
							// Creates the new group.
							final LogFrameGroupDTO group = logFrame.addGroup(
									label, LogFrameGroupType.EXPECTED_RESULT);
	 ...
	 * @return The just created group.
	 */
	private RowsGroup<LogFrameGroupDTO> addExpectedResultsGroup(
			final LogFrameGroupDTO group) {
		...
		final RowsGroup<LogFrameGroupDTO> g = new RowsGroup<LogFrameGroupDTO>(
			...
			public Widget getWidget() {
				...
				if (!readOnly) {
					grid.setWidget(0, 1,
							buildGroupMenu(expectedResultsView, this, label));
				}
	...
	protected void addExpectedResult() {
		...
		if (max != null && max > 0 && max <= expectedResultsView.getRowsCount()) {
			MessageBox.alert(I18N.CONSTANTS.logFrameUnauthorizedAction(),
					I18N.CONSTANTS.logFrameMaxReachedER(), null);
			return;
		}

		final List<SpecificObjectiveDTO> objectives = logFrame
				.getSpecificObjectivesDTONotDeleted();
		...
		if (logFrameModel.getEnableExpectedResultsGroups()) {
			...
			// Sets the form window.
			formWindow.clear();
			formWindow.addChoicesList(
					I18N.CONSTANTS.logFrameSpecificObjective(), objectives,
					false, "label");
			formWindow.addChoicesList(I18N.CONSTANTS.logFrameGroup(),
					logFrame.getAllGroups(LogFrameGroupType.EXPECTED_RESULT),
					false, "label");
			formWindow.addFormSubmitListener(new FormSubmitListener() {
				...
				public void formSubmitted(Object... elements) {
					...
					// Creates and displays a new objective.
					final ExpectedResultDTO result = specificObjective
							.addExpectedResult();
			...
			formWindow.show(I18N.CONSTANTS.logFrameAddER(),
					I18N.CONSTANTS.logFrameSelectGroupER());
		}
		// Groups are disabled, no need to select a group, the default one will
		// be selected.
		else {
			...
			formWindow.clear();
			formWindow.addChoicesList(
					I18N.CONSTANTS.logFrameSpecificObjective(),
					logFrame.getSpecificObjectivesDTONotDeleted(), false,
					"label");
			formWindow.addFormSubmitListener(new FormSubmitListener() {
				...
				public void formSubmitted(Object... elements) {
					...
					// Retrieves the default group.
					final LogFrameGroupDTO group = logFrame
							.getDefaultGroup(LogFrameGroupType.EXPECTED_RESULT);
			...
			formWindow.show(I18N.CONSTANTS.logFrameAddER(),
					I18N.CONSTANTS.logFrameSelectGroup2ER());
		}
	}
	...
	private void addExpectedResult(final ExpectedResultDTO result) {
		...
		// Adds the row.
		expectedResultsView.insertRow(result.getPosition(),
				group.getClientSideId(), new Row<ExpectedResultDTO>(result) {

					@Override
					public boolean isSimilar(int column,
							ExpectedResultDTO userObject,
							ExpectedResultDTO other) {
					...
					@Override
					public Widget getWidgetAt(int column,
							final ExpectedResultDTO userObject) {
						...
						case 0:
							...
							parentCodeLabel
									.addStyleName(CSS_CODE_LABEL_STYLE_NAME);

							final SpecificObjectiveDTO parent;
							if (userObject != null
									&& (parent = userObject
											.getParentSpecificObjectiveDTO()) != null) {

								final StringBuilder sb = new StringBuilder();

								sb.append(I18N.CONSTANTS
										.logFrameExceptedResultsCode());
								sb.append(" (");
								sb.append(I18N.CONSTANTS
										.logFrameSpecificObjectivesCode());
								sb.append(" ");
								sb.append(specificObjectivesPolicy.getCode(
										parent.getCode(), parent));
								sb.append(")");
						...
						case 1:
							...
							if (!readOnly) {
								grid.setWidget(
										0,
										1,
										buildExpectedResultMenu(this, codeLabel));
							}
		...
		if (result.getActivitiesDTONotDeleted() != null) {
			for (final LogFrameActivityDTO activity : result
					.getActivitiesDTONotDeleted()) {
	...
	protected void addActivitiesGroup() {
		...
		if (!logFrameModel.getEnableActivitiesGroups()) {
			MessageBox.alert(I18N.CONSTANTS.logFrameUnauthorizedAction(),
					I18N.CONSTANTS.logFrameGroupsDisabledA(), null);
			return;
		}
		...
		if (max != null && max > 1 && max <= activitiesView.getGroupsCount()) {
			MessageBox.alert(I18N.CONSTANTS.logFrameUnauthorizedAction(),
					I18N.CONSTANTS.logFrameGroupsMaxReachedA(), null);
			return;
		}

		// Asks for the new group label.
		MessageBox.prompt(I18N.CONSTANTS.logFrameAddGroup(),
				I18N.CONSTANTS.logFrameAddGroupToA(), false,
				new Listener<MessageBoxEvent>() {
					...
					public void handleEvent(MessageBoxEvent be) {
						...
						if (Dialog.OK.equals(be.getButtonClicked().getItemId())) {
							...
							// Creates the new group.
							final LogFrameGroupDTO group = logFrame.addGroup(
									label, LogFrameGroupType.ACTIVITY);
	 ...
	 * @return The just created group.
	 */
	private RowsGroup<LogFrameGroupDTO> addActivitiesGroup(
			final LogFrameGroupDTO group) {
		...
		final RowsGroup<LogFrameGroupDTO> g = new RowsGroup<LogFrameGroupDTO>(
			...
			public Widget getWidget() {
				...
				if (!readOnly) {
					grid.setWidget(0, 1,
							buildGroupMenu(activitiesView, this, label));
				}
	...
	protected void addActivity() {
		...
		if (max != null && max > 0 && max <= activitiesView.getRowsCount()) {
			MessageBox.alert(I18N.CONSTANTS.logFrameUnauthorizedAction(),
					I18N.CONSTANTS.logFrameMaxReachedA(), null);
			return;
		}

		final List<ExpectedResultDTO> results = logFrame
				.getAllExpectedResultsDTO();
		...
		if (logFrameModel.getEnableActivitiesGroups()) {
			...
			// Sets the form window.
			formWindow.clear();
			formWindow.addChoicesList(I18N.CONSTANTS.logFrameExceptedResult(),
					results, false, "label");
			formWindow.addChoicesList(I18N.CONSTANTS.logFrameGroup(),
					logFrame.getAllGroups(LogFrameGroupType.ACTIVITY), false,
					"label");
			formWindow.addDateField(I18N.CONSTANTS.logFrameActivityStartDate(),
					false);
			formWindow.addDateField(I18N.CONSTANTS.logFrameActivityEndDate(),
					false);
			formWindow.addTextField(I18N.CONSTANTS.logFrameActivityContent(),
					true);
			formWindow.addFormSubmitListener(new FormSubmitListener() {
				...
				public void formSubmitted(Object... elements) {
					...
					final LogFrameActivityDTO activity = expectedResult
							.addActivity();
			...
			formWindow.show(I18N.CONSTANTS.logFrameAddA(),
					I18N.CONSTANTS.logFrameSelectGroupA());
		}
		// Groups are disabled, no need to select a group, the default one will
		// be selected.
		else {
			...
			formWindow.addChoicesList(I18N.CONSTANTS.logFrameExceptedResult(),
					results, false, "label");
			formWindow.addTextField(I18N.CONSTANTS.logFrameActivityTitle(),
					false);
			formWindow.addDateField(I18N.CONSTANTS.logFrameActivityStartDate(),
					false);
			formWindow.addDateField(I18N.CONSTANTS.logFrameActivityEndDate(),
			...
			formWindow.addFormSubmitListener(new FormSubmitListener() {
				...
				public void formSubmitted(Object... elements) {
					...
					final ExpectedResultDTO expectedResult = (ExpectedResultDTO) element0;
					final LogFrameGroupDTO group = logFrame
							.getDefaultGroup(LogFrameGroupType.ACTIVITY);
			...
			formWindow.show(I18N.CONSTANTS.logFrameAddA(),
					I18N.CONSTANTS.logFrameSelectGroup2A());
		}
	}
	...
	private void addActivity(final LogFrameActivityDTO activity) {
		...
		// Adds the row.
		activitiesView.insertRow(activity.getPosition(),
				group.getClientSideId(),
				new Row<LogFrameActivityDTO>(activity) {

					@Override
					public boolean isSimilar(int column,
							LogFrameActivityDTO userObject,
						...
						case 0:
							...
							return userObject.getParentExpectedResultDTO() != null
									...
									&& userObject.getParentExpectedResultDTO()
											.getCode() == other
					...
					@Override
					public Widget getWidgetAt(int column,
							final LogFrameActivityDTO userObject) {
						...
						case 0:
							...
							if (userObject != null
									&& (parent = userObject
											.getParentExpectedResultDTO()) != null) {

								final StringBuilder sb = new StringBuilder();

								sb.append(I18N.CONSTANTS
										.logFrameActivitiesCode());
								sb.append(" (");
								sb.append(I18N.CONSTANTS
										.logFrameExceptedResultsCode());
								sb.append(" ");
								sb.append(expectedResultsPolicy.getCode(
										parent.getCode(), parent));
								sb.append(")");
						...
						case 1:
							...
							if (!readOnly) {
								grid.setWidget(0, 1,
										buildActivityMenu(this, codeLabel));
							}
						...
						case 5:
							...
							contentTextBox
									.addChangeHandler(new ChangeHandler() {
										@Override
										public void onChange(ChangeEvent e) {
											userObject.setTitle(contentTextBox
													.getText());
											fireLogFrameEdited();
										}
									});
	...
	protected void addPrerequisitesGroup() {
		...
		if (!logFrameModel.getEnablePrerequisitesGroups()) {
			MessageBox.alert(I18N.CONSTANTS.logFrameUnauthorizedAction(),
					I18N.CONSTANTS.logFrameGroupsDisabledP(), null);
			return;
		}
		...
		if (max != null && max > 1 && max <= prerequisitesView.getGroupsCount()) {
			MessageBox.alert(I18N.CONSTANTS.logFrameUnauthorizedAction(),
					I18N.CONSTANTS.logFrameGroupsMaxReachedP(), null);
			return;
		}

		// Asks for the new group label.
		MessageBox.prompt(I18N.CONSTANTS.logFrameAddGroup(),
				I18N.CONSTANTS.logFrameAddGroupToP(), false,
				new Listener<MessageBoxEvent>() {
					...
					public void handleEvent(MessageBoxEvent be) {
						...
						if (Dialog.OK.equals(be.getButtonClicked().getItemId())) {
							...
							// Creates the new group.
							final LogFrameGroupDTO group = logFrame.addGroup(
									label, LogFrameGroupType.PREREQUISITE);
	 ...
	 * @return The just created group.
	 */
	private RowsGroup<LogFrameGroupDTO> addPrerequisitesGroup(
			final LogFrameGroupDTO group) {
		...
		final RowsGroup<LogFrameGroupDTO> g = new RowsGroup<LogFrameGroupDTO>(
			...
			public Widget getWidget() {
				...
				if (!readOnly) {
					grid.setWidget(0, 1,
							buildGroupMenu(prerequisitesView, this, label));
				}
	...
	protected void addPrerequisite() {
		...
		if (max != null && max > 0 && max <= prerequisitesView.getRowsCount()) {
			MessageBox.alert(I18N.CONSTANTS.logFrameUnauthorizedAction(),
					I18N.CONSTANTS.logFrameMaxReachedP(), null);
			return;
		}
		...
		if (logFrameModel.getEnablePrerequisitesGroups()) {
			...
			formWindow.clear();
			formWindow.addChoicesList(I18N.CONSTANTS.logFrameGroup(),
					logFrame.getAllGroups(LogFrameGroupType.PREREQUISITE),
					false, "label");
			formWindow.addFormSubmitListener(new FormSubmitListener() {
				...
				public void formSubmitted(Object... elements) {
					...
					final PrerequisiteDTO prerequisite = logFrame
							.addPrerequisite();
			...
			formWindow.show(I18N.CONSTANTS.logFrameAddP(),
					I18N.CONSTANTS.logFrameSelectGroupP());
		}
		// Groups are disabled, no need to select a group, the default one will
		// be selected.
		else {
			...
			// Retrieves the default group.
			final LogFrameGroupDTO group = logFrame
					.getDefaultGroup(LogFrameGroupType.PREREQUISITE);
	...
	private void addPrerequisite(final PrerequisiteDTO prerequisite) {
		...
		sb.append(I18N.CONSTANTS.logFramePrerequisitesCode());
		sb.append(" ");
		sb.append(prerequisitesPolicy.getCode(prerequisite.getCode(),
				prerequisite));
		...
		// Adds the row.
		prerequisitesView.insertRow(prerequisite.getPosition(),
				group.getClientSideId(),
				new Row<PrerequisiteDTO>(prerequisite) {

					@Override
					public boolean isSimilar(int column,
							PrerequisiteDTO userObject, PrerequisiteDTO other) {
						return false;
					}

					@Override
					public Widget getWidgetAt(int column,
							final PrerequisiteDTO userObject) {
						...
						case 0:
							...
							if (!readOnly) {
								grid.setWidget(0, 1,
										buildPrerequisiteMenu(this, codeLabel));
							}
	 ...
	 * @return The menu.
	 */
	private Widget buildSpecificObjectiveMenu(
			final Row<SpecificObjectiveDTO> row, final Label label) {
		...
		return buildMenuWidget(new RowActionsMenu(specificObjectivesView, row) {
			...
			public boolean canBeRemoved() {
				...
				final List<ExpectedResultDTO> results = row.getUserObject()
						.getExpectedResultsDTONotDeleted();
			...
			public boolean beforeRemove() {
				...
				final boolean removed = logFrame.removeSpecificObjective(row
						.getUserObject());
	...
	private Widget buildExpectedResultMenu(final Row<ExpectedResultDTO> row,
		...
		return buildMenuWidget(new RowActionsMenu(expectedResultsView, row) {
			...
			public boolean canBeRemoved() {
				...
				final List<LogFrameActivityDTO> activities = row
						.getUserObject().getActivitiesDTONotDeleted();
	...
	private Widget buildPrerequisiteMenu(final Row<PrerequisiteDTO> row,
		...
		return buildMenuWidget(new RowActionsMenu(prerequisitesView, row) {
			...
			public boolean beforeRemove() {
				...
				final boolean removed = logFrame.removePrerequisite(row
						.getUserObject());
	 ...
	 * @return The menu.
	 */
	private Widget buildGroupMenu(final FlexTableView view,
			final RowsGroup<LogFrameGroupDTO> group, final Label label) {
		...
		return buildMenuWidget(new GroupActionMenu(view, group) {
			...
			public void beforeRename(final AsyncCallback<String> callback) {
				...
				MessageBox.prompt(I18N.CONSTANTS.logFrameActionRenameGroup(),
						I18N.CONSTANTS.logFrameActionRenameNewName(), false,
						new Listener<MessageBoxEvent>() {
							...
							public void handleEvent(MessageBoxEvent be) {
								...
								// OK.
								if (Dialog.OK.equals(be.getButtonClicked()
										.getItemId())) {
