...
public class ProjectDashboardPresenter implements SubPresenter {
				...
				public void handleEvent(ComponentEvent tpe) {
					...
					if (view.getButtonSavePhase().isEnabled()
						...
						MessageBox.confirm(I18N.CONSTANTS.projectPhaseChangeAlert(),
						                I18N.CONSTANTS.projectPhaseChangeAlertDetails(),
						                new Listener<MessageBoxEvent>() {

							                @Override
							                public void handleEvent(MessageBoxEvent ce) {

								                // If 'YES' is clicked, saves
								                // the
								                // modifications.
								                if (Dialog.YES.equals(ce.getButtonClicked().getItemId())) {
									                ...
									                // If the last displayed
									                // phase was
									                // the active one,
									                // modifications are
									                // discarded then the
									                // required
									                // elements map is cleared
									                // (to
	...
	private void activatePhase(final PhaseDTO phase, final boolean reload) {
			                ...
			                I18N.MESSAGES.projectEnd(projectPresenter.getCurrentProjectDTO().getCurrentPhaseDTO()
				                ...
				                public void handleEvent(MessageBoxEvent be) {
					                ...
					                if (Dialog.YES.equals(be.getButtonClicked().getItemId())) {
						                ...
						                // Activates the current displayed
						                // phase.
						                dispatcher.execute(new ChangePhase(projectPresenter.getCurrentProjectDTO()
						                                .getId(), null), null, new AsyncCallback<ProjectDTO>() {
							                ...
							                public void onSuccess(ProjectDTO result) {
								                ...
								                // displayed phase
								                // (not necessary the active
								                // one).
								                for (final PhaseDTO phase : projectPresenter.getCurrentProjectDTO()
								                                .getPhasesDTO()) {
									                if (phase.getId() == projectPresenter.getCurrentDisplayedPhaseDTO()
									                                .getId()) {
		...
		// Else the active will be closed and the new phase will be activated.
			...
			MessageBox.confirm(I18N.CONSTANTS.projectCloseAndActivate(), I18N.MESSAGES.projectCloseAndActivate(
			                projectPresenter.getCurrentProjectDTO().getCurrentPhaseDTO().getPhaseModelDTO().getName(),
			                phase.getPhaseModelDTO().getName()), new Listener<MessageBoxEvent>() {
				...
				public void handleEvent(MessageBoxEvent be) {
					...
					if (Dialog.YES.equals(be.getButtonClicked().getItemId())) {
						...
						dispatcher.execute(
						                new ChangePhase(projectPresenter.getCurrentProjectDTO().getId(), phase.getId()),
			...
			dispatcher.execute(updateProject,
			                ...
			                new AsyncCallback<VoidResult>() {
				                ...
				                public void onSuccess(VoidResult result) {
					                ...
					                // Checks if there is any update needed to
					                // the local
					                // project instance.
			...
			public void handleEvent(ButtonEvent be) {
				...
				dispatcher.execute(cmd, null, new AsyncCallback<ProjectListResult>() {
					...
					public void onSuccess(ProjectListResult result) {
						...
						// project and classify the projects by country
						final Map<String, List<ProjectDTOLight>> map = new HashMap<String, List<ProjectDTOLight>>();
						for (final ProjectDTOLight p : resultList) {
							p.generateCompleteName();
							List<ProjectDTOLight> pjo = map.get(p.getCountryName());
							if (pjo == null) {
								pjo = new ArrayList<ProjectDTOLight>();
							}
							p.generateTypeIconHTML(authentication.getOrganizationId());
							pjo.add(p);
							map.put(p.getCountryName(), pjo);
						}
						...
						for (final String key : map.keySet()) {
							...
							for (ProjectDTOLight pdo : map.get(key)) {
								projectsListForCombo.add(pdo);
							}
						}
						...
						// Adds fields.
						final ComboBox<ModelData> projects = window.addChoicesList(
						                I18N.CONSTANTS.createProjectTypeFunding(), projectsListForCombo, false,
						                "completeName");

						projects.setTemplate("<tpl for=\".\">"
						                + "<tpl if=\"!values.completeName\">"
						                + "<div class=\"x-combo-list-item x-combo-list-item-disabled\">{values.name}</div>"
						                + "</tpl>"
						                + "<tpl if=\"values.completeName\">"
						                + "<div class=\"x-combo-list-item\">{values.typeIconHtml} {values.completeName}</div>"
						                + "</tpl>" + "</tpl>");
						...
						projects.addListener(Events.Select, new Listener<BaseEvent>() {
							...
							public void handleEvent(BaseEvent be) {
								if (!projects.getSelection().isEmpty()
								                && projects.getSelection().get(0) instanceof ProjectDTOLight) {
									final ProjectDTOLight pdo = (ProjectDTOLight) projects.getSelection().get(0);
									final ProjectModelType type = pdo.getProjectModelType(authentication
									                .getOrganizationId());
									...
									iconGrid.setWidget(0, 0,
									                FundingIconProvider.getProjectTypeIcon(type, IconSize.MEDIUM)
									                                .createImage());
									DOM.setStyleAttribute(iconGrid.getCellFormatter().getElement(0, 0), "paddingTop",
									                "2px");
								                ...
								                new AsyncCallback<CreateResult>() {
									                ...
									                public void onFailure(Throwable e) {
										                ...
										                MessageBox.alert(
										                                ...
										                                I18N.CONSTANTS.createProjectTypeFundingCreationDetails(),
										                                null);
									                }
									                ...
									                public void onSuccess(CreateResult result) {
										                ...
										                final ProjectFundingDTO r = (ProjectFundingDTO) result
										                                .getEntity();
			...
			public void handleEvent(ButtonEvent be) {
				...
				dispatcher.execute(cmd, null, new AsyncCallback<ProjectListResult>() {
					...
					public void onSuccess(ProjectListResult result) {
						...
						// Adds fields.
						final ComboBox<ModelData> projects = window.addChoicesList(
						                I18N.CONSTANTS.createProjectTypePartner(), projectsListForCombo, false,
						                "completeName");
						...
						amountField.addListener(Events.Change, new Listener<BaseEvent>() {
							...
							public void handleEvent(BaseEvent be) {
								...
								final List<ModelData> selection = projects.getSelection();

								if (selection == null || selection.isEmpty()
								                || (!(selection.get(0) instanceof ProjectDTOLight))) {
									percentageField.setText(I18N.CONSTANTS.createProjectPercentageNotAvailable());
								} else {
									ProjectDTOLight pdo = (ProjectDTOLight) selection.get(0);
									percentageField.setText(NumberUtils.ratioAsString(amountField.getValue(),
									                pdo.getPlannedBudget()));
								}
							}
						});
							                ...
							                new AsyncCallback<CreateResult>() {
								                ...
								                public void onFailure(Throwable e) {
									                ...
									                MessageBox.alert(I18N.CONSTANTS.monitoredPointAddError(),
									                                I18N.CONSTANTS.monitoredPointAddErrorDetails(),
									                                null);
								                }
								                ...
								                public void onSuccess(CreateResult result) {
									                ...
									                final MonitoredPointDTO point = (MonitoredPointDTO) result
									                                .getEntity();

									                // Gets the project list and
									                // creates
									                // it
									                // if needed.
									                MonitoredPointListDTO list = projectPresenter
									                                .getCurrentProjectDTO().getPointsList();
		                ...
		                .addListener(Store.Update, new Listener<StoreEvent<MonitoredPointDTO>>() {
			                ...
			                public void handleEvent(StoreEvent<MonitoredPointDTO> se) {
				                ...
				                if (se.getOperation() == RecordUpdate.EDIT) {
					                ...
					                // The 'completed' field has been edited by
					                // the grid
					                // editor, but the actual property which is
					                // saved in
					                // data-layer is 'completionDate'. we have
					                // to do the
					                // changes manually.
					                ...
					                dispatcher.execute(new UpdateMonitoredPoints(editedModels),
					                                new MaskingAsyncMonitor(view.getMonitoredPointsGrid(),
					                                ...
					                                new AsyncCallback<MonitoredPointsResultList>() {
						                                ...
						                                public void onFailure(Throwable e) {
							                                ...
							                                Log.error("[execute] Error while merging the monitored points.",
							                                                e);
							                                MessageBox.alert(
							                                                I18N.CONSTANTS.monitoredPointUpdateError(),
							                                                I18N.CONSTANTS.monitoredPointUpdateErrorDetails(),
							                                                null);
						                                }
	...
	private void refreshRequiredElementContentPanelHeader() {
		...
		view.getRequiredElementContentPanel().setHeading(
		                I18N.CONSTANTS.projectRequiredElements() + " (" + filledRequiredElements + "/"
