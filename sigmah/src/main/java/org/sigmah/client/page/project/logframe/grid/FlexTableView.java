...
public class FlexTableView {
	 ...
	 */
	private static final String CSS_GROUP_CELL_STYLE_NAME = CSS_FLEX_TABLE_VIEW_STYLE_NAME
			+ "-group-cell";

	/**
	 * CSS style name for the labels which display groups.
	 */
	private static final String CSS_GROUP_LABEL_STYLE_NAME = CSS_FLEX_TABLE_VIEW_STYLE_NAME
			+ "-group-label";

	/**
	 * CSS style name for the merged rows.
	 */
	private static final String CSS_MERGED_ROWS_STYLE_NAME = CSS_FLEX_TABLE_VIEW_STYLE_NAME
			+ "-merged-row";
	...
	public FlexTableView(FlexTable table, int columnsCount, int row) {
		...
		if (row < 0 || row >= table.getRowCount()) {
			throw new IllegalArgumentException(
					"the flex table does not have a row at index #" + row + ".");
		}

		// Checks if the table contains enough columns.
		if (columnsCount < 2) {
			throw new IllegalArgumentException(
					"the flex table does not contains enought columns (min 2).");
		}
	...
	public void addDependency(FlexTableView other) {
		...
		if (table != other.table) {
			throw new IllegalArgumentException(
					"the other view doesn't share the same table as the current view");
		}
	...
	protected int insertTableRow(int beforeRow) {
		...
		table.getFlexCellFormatter().setRowSpan(
				...
				table.getFlexCellFormatter()
						.getRowSpan(shift, groupColumnIndex) + 1);
	...
	/**
	 * Remove a log group when it is empty (should be verified before this
	 * action)
	 * 
	 * @param a
	 *            RowsGroup
	...
	public void removeGroup(final RowsGroup<?> group) {
		...
		// Remove the row from the table,the shift is considered in the
		// removeTableRow method
	...
	public void insertGroup(int position, final RowsGroup<?> group) {
		...
		if (groupsCodesMap.get(id) != null) {
			throw new IllegalArgumentException("The group with id #" + id
					+ " already exists.");
		}
		...
		if (Log.isDebugEnabled()) {
			Log.debug("[insertGroup] Inserts the new group #" + id
		...
		table.getFlexCellFormatter().addStyleName(row, column,
				CSS_GROUP_CELL_STYLE_NAME);
	...
	protected int getGroupPosition(final RowsGroup<?> group) {
		...
		if (position == -1) {
			throw new IllegalArgumentException("The group with id #"
					+ group.getId() + " doesn't exist.");
		}
	...
	public void refreshGroupWidget(final RowsGroup<?> group) {
		...
		if (Log.isDebugEnabled()) {
			Log.debug("[refreshGroupWidget] Refreshes the group #"
					+ group.getId() + ".");
		}
	...
	public <T> void insertRow(int position, final int groupId, final Row<T> row) {
		...
		if (Log.isDebugEnabled()) {
			Log.debug("[insertRow] Inserts the new row #" + row.getId()
					+ " in group #" + groupId + " at position #" + position
					+ ".");
		}
		...
		for (int j = 0; j < columnsCount; j++) {
			...
			if (w == null) {
				...
				if (colSpan != 0) {
					table.getFlexCellFormatter().setColSpan(rowIndex, column,
							colSpan + 1);
				}

				HTMLTableUtils.applyCellStyles(table, rowIndex, column, false,
						false);
			...
			if (merge.contains(j)) {
				...
				if ((topRow = group.getRowAtPosition(position - 1)) != null) {
					...
					if (row.isSimilar(j, row.getUserObject(),
							((Row<T>) topRow).getUserObject())) {
						table.setWidget(rowIndex, column, new Label(""));
						table.getFlexCellFormatter().addStyleName(rowIndex,
								column, CSS_MERGED_ROWS_STYLE_NAME);
					}
				}
			}
	...
	public void removeRow(final RowsGroup<?> group, final int rowId) {
		...
		if ((row = group.getRow(rowId)) == null) {
			throw new IllegalArgumentException("The row with id #" + rowId
					+ " does'nt exist in group #" + group.getId() + ".");
		}
	...
	private void refreshMergedRow(RowsGroup<?> group, int position)
			throws IndexOutOfBoundsException {
		...
		for (int j = 0; j < columnsCount; j++) {
			...
			if (merge.contains(j)) {
				...
				if ((topRow = group.getRowAtPosition(position - 1)) != null) {
					...
					// If the rows properties are similar, removes the widget.
					if (row.isSimilar(j, row.getUserObject(),
							topRow.getUserObject())) {
						table.setWidget(rowIndex, column, new Label(""));
						table.getFlexCellFormatter().addStyleName(rowIndex,
								column, CSS_MERGED_ROWS_STYLE_NAME);
					} else {
						table.setWidget(rowIndex, column, w);
						HTMLTableUtils.applyCellStyles(table, rowIndex, column,
								false, false);
						table.getFlexCellFormatter().removeStyleName(rowIndex,
								column, CSS_MERGED_ROWS_STYLE_NAME);
					}
				} else {
					table.setWidget(rowIndex, column, w);
	...
	public void moveRow(final RowsGroup<?> group, final int rowId, int move) {
		...
		final Row<?> row;
		if ((row = group.getRow(rowId)) == null) {
			throw new IllegalArgumentException("The row #" + rowId
					+ " does'nt exist in group #" + group.getId() + ".");
		}
	 ...
	 *             If the row doesn't exist.
	 */
	public boolean canBeMoved(final RowsGroup<?> group, final int rowId,
			int move) {
