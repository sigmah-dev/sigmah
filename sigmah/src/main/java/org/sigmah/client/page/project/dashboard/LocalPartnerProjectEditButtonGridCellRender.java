...
/**
 * The render for edit icon button in LocalPartnerProjectGrid.
...
public class LocalPartnerProjectEditButtonGridCellRender implements GridCellRenderer<ProjectFundingDTO> {
	...
	public LocalPartnerProjectEditButtonGridCellRender(ProjectDashboardView view,
			...
			public void onClick(ClickEvent event) {
		       ...
		       window.addFormSubmitListener(new FormSubmitListener(){
				...
				public void formSubmitted(Object... values) {
					...
					//Update 
                     dispatcher.execute(new UpdateEntity("ProjectFunding",model.getId(),changes), new MaskingAsyncMonitor(view.getLocalPartnerProjectGrid(), I18N.CONSTANTS.loading()),  new AsyncCallback<VoidResult>(){
				           ...
				           public void handleEvent(MessageBoxEvent be) {  
				        	   ...
				        	   if(btn.getText().equals(I18N.CONSTANTS.yes()))
				        	    {				        		            
			                       ...
			                       //RPC
							        dispatcher.execute(new UpdateEntity("Project",projectPresenter.getCurrentProjectDTO().getId(),properties), new MaskingAsyncMonitor(view.getLocalPartnerProjectGrid(), I18N.CONSTANTS.loading()),
							        		new AsyncCallback<VoidResult>(){
												...
												view.getLocalPartnerProjectGrid().getStore().remove(model);																																																																											     
												window.hide();
												Notification.show(
																I18N.CONSTANTS
																		.infoConfirmation(),
