...
public class AdminOneModelPresenter{
	 ...
	 * 
	 * @return
	 *        {@link org.sigmah.client.page.admin.model.common.StatusChangeAction} object
	 *        
	...
	private StatusChangeAction isValidStatusChange(ProjectModelStatus currentStatus, ProjectModelStatus targetStatus)	
	{
		...
		//"Ready" model is only allowed to shift to "Draft" or "Unavailable" model
		else if(currentStatus.equals(ProjectModelStatus.READY) && !(  targetStatus.equals(ProjectModelStatus.DRAFT)  || targetStatus.equals(ProjectModelStatus.UNAVAILABLE) ) )
		{
			statusChangeAction.setValid(false);
	...
	private void isValidUnavailableStatusChange(final ProjectModelStatus targetStatus) {
		...
		dispatcher.execute(checkCommand, null, new AsyncCallback<ModelCheckResult>(){
			...
			public void onFailure(Throwable arg0) {
				...
				MessageBox.alert(I18N.CONSTANTS.adminModelCheckError(), I18N.CONSTANTS.adminModelCheckErrorDetails(), null);
				
				if(checkCommand.getModelType().equals(CheckModelUsage.ModelType.ProjectModel))
				{
					view.getStatusList().setSimpleValue(ProjectModelStatus.getName(view.getCurrentProjectModel().getStatus()));
					
				}
				else
				{
					view.getStatusList().setSimpleValue(ProjectModelStatus.getName(view.getCurrentOrgUnitModel().getStatus()));
				}
			...
			public void onSuccess(ModelCheckResult result) {
				...
				if(result==null || result.isUsed()==null)
				{//Row back value if RPC encounters errors
				...
				{//RPC succeeds 
					...
					//"Unavailable" model that is ever used is only allowed to shift back to "Used" model.
					if(result.isUsed() && !targetStatus.equals(ProjectModelStatus.USED))
					{
						
						//Row back value 
						
						MessageBox.alert(I18N.CONSTANTS.error(), I18N.CONSTANTS.unavailableUsedModelStatusChangeError(), null);
					...
					//"Unavailable" model that is never used is only allowed to shift back to "Ready" model.
					else if(!result.isUsed() && !targetStatus.equals(ProjectModelStatus.READY))
					{
                        //Row back value 
						
						MessageBox.alert(I18N.CONSTANTS.error(), I18N.CONSTANTS.unavailableNotUsedModelStatusChangeError(), null);
	...
	private void onOrgUnitModelStatusChanged() {
		...
		if(!view.getCurrentOrgUnitModel().getStatus().equals(ProjectModelStatus.UNAVAILABLE))
		  {
		      StatusChangeAction statusChangeAction=isValidStatusChange(view.getCurrentOrgUnitModel().getStatus(),ProjectModelStatus.getStatus(view.getStatusList().getValue().getValue()));
		  	  
		      if(!statusChangeAction.isValid())
		      {//Unvalid status changing action,rollback value
		    	  MessageBox.alert(I18N.CONSTANTS.error(), statusChangeAction.getFeedBackMessage(), null);
		    	  view.getStatusList().setSimpleValue(ProjectModelStatus.getName(view.getCurrentOrgUnitModel().getStatus()));
		    	  return;
		      }
		      else
		      {//Valid status changing action
		    	  ...
		    	  {
		    		  onDraftOrgUnitModelStatusChange(ProjectModelStatus.getStatus(view.getStatusList().getValue().getValue()));
		    	  }
		    	  else
		    	  {
		    		  //Do nothing
		  ...
		  else if (view.getCurrentOrgUnitModel().getStatus().equals(ProjectModelStatus.UNAVAILABLE))
		      ...
		      {//Valid status changing action
		    	  ...
		    	  if(view.getCurrentProjectModel().getStatus().equals(ProjectModelStatus.DRAFT))
		    	  {
		    		  ...
		    		  MessageBox.confirm(I18N.MESSAGES.adminModelStatusChangeBox(), 
		    					I18N.MESSAGES.adminModelDraftStatusChange(ProjectModelStatus.getName(targetStatus)), new Listener<MessageBoxEvent>(){
		    	  ...
		    	  else
		    	  {
		    		  ...
		    		  Log.debug("Current status: "+ProjectModelStatus.getName(view.getCurrentOrgUnitModel().getStatus())+ " target status: "+view.getStatusList().getValue().getValue());
		    		  //Do nothing
		    	  }
		      }
		  }
		  else if (view.getCurrentProjectModel().getStatus().equals(ProjectModelStatus.UNAVAILABLE))
		  {//"Unavailable" model needs to be checked in a different way
			  isValidUnavailableStatusChange(ProjectModelStatus.getStatus(view.getStatusList().getValue().getValue()));
		  }
	...
	private void onProjectModelStatusChanged() {
	  ...
	  {
	      StatusChangeAction statusChangeAction=isValidStatusChange(view.getCurrentProjectModel().getStatus(),ProjectModelStatus.getStatus(view.getStatusList().getValue().getValue()));
	      ...
	      {//Valid status changing action
	    	  ...
	    	  if(view.getCurrentProjectModel().getStatus().equals(ProjectModelStatus.DRAFT))
	    	  {
	    		  ...
	    		  onDraftProjectModelStatusChange(ProjectModelStatus.getStatus(view.getStatusList().getValue().getValue()));
	    	  }
	    	  else
	    	  {
	    		  Log.debug("non draft model status is changing !");
	    		  Log.debug("Current status: "+ProjectModelStatus.getName(view.getCurrentProjectModel().getStatus())+ " target status: "+view.getStatusList().getValue().getValue());
	    		  //Do nothing
	    	  }
	      }
	  }
	   ...
	   MessageBox.confirm(I18N.MESSAGES.adminModelStatusChangeBox(), 
             ...
             public void handleEvent(MessageBoxEvent be) {
				...
				else 
				{
					...
					dispatcher.execute(cmdGetProjectsByModel, new MaskingAsyncMonitor(view.getTopPanel(), I18N.CONSTANTS.loading()), new AsyncCallback<ProjectListResult>(){
						...
						public void onSuccess(
							...
							if(result!=null && testProjects!=null && testProjects.size()>0 )
							{
								...
								String waringMessage = I18N.MESSAGES.DraftProjectModelChangeStatusDetails(testProjectNames,ProjectModelStatus.getName(targetStatus));
...
private void updateModel(){
	...
	if(view.isProject()){
		if(view.getNameField().getValue() == null || view.getStatusList().getValue() == null || view.getCurrentModelType() == null ){
		...
		dispatcher.execute(new CreateEntity("ProjectModel", modelProperties), null, new AsyncCallback<CreateResult>(){
            public void onFailure(Throwable caught) {  
            	MessageBox.alert(I18N.CONSTANTS.adminProjectModelUpdateBox(), 
              			I18N.MESSAGES.adminStandardCreationFailure(I18N.CONSTANTS.adminProjectModelStandard()
 								+ " '" + view.getNameField().getValue() + "'"), null);
            }
			...
			public void onSuccess(CreateResult result) {
				if(result != null && result.getEntity() != null){
					...
					eventBus.fireEvent(new NavigationEvent(NavigationHandler.NavigationRequested, currentState
                            .deriveTo(currentState.getCurrentSection(), currentState.getModel(), MAIN_TABS[tabIndex], true)));
        ...
        dispatcher.execute(new CreateEntity("OrgUnitModel", modelProperties), null, new AsyncCallback<CreateResult>(){
			...
			public void onSuccess(CreateResult result) {
				if(result != null && result.getEntity() != null){	
					...
					//Refresh the page
					eventBus.fireEvent(new NavigationEvent(NavigationHandler.NavigationRequested, currentState
                            .deriveTo(currentState.getCurrentSection(), currentState.getModel(), MAIN_TABS[tabIndex], false)));
