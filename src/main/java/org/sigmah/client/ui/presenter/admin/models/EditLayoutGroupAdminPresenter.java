...
public class EditLayoutGroupAdminPresenter extends AbstractPagePresenter<EditLayoutGroupAdminPresenter.View> implements HasForm {
     ...
     *
     * @param view The view managed by the presenter.
    ...
    public void onPageRequest(final PageRequest request) {
        ...
        layoutGroup = request.getData(RequestParameter.DTO);
		...
		if (layoutGroup != null) {
            ...
            view.getContainerField().setValue(layoutGroup.getContainerModel());
            setRowFieldValues(layoutGroup.getContainerModel(), layoutGroup.getRow());
        } else {
            layoutGroup = null;
        }
    }
    ...
    // ---------------------------------------------------------------------------------------------------------------
    /**
     * Populates the row field with the given {@code hasLayout} corresponding
     * layout row counts.
     *
     * @param hasLayout The component with a layout.
     * @param selectedValue The optional selected value.
     */
     ...
     *
     * @param hasLayout The container, may be {@code null}.
     * @return The given {@code hasLayout} corresponding {@link LayoutDTO}, or
     * {@code null}.
     */
	...
	// TODO : fix the unexpected error. 
        ...
        dispatch.execute(new CreateEntity(LayoutGroupDTO.ENTITY_NAME, newGroupProperties), new CommandResultHandler<CreateResult>() {
            ...
            public void onCommandSuccess(final CreateResult result) {
                ...
                final LayoutGroupDTO lg = (LayoutGroupDTO) result.getEntity();
                if (layoutGroup != null) {
                    // Update case.
                    N10N.infoNotif(I18N.CONSTANTS.adminPhaseCreationBox(),
                            I18N.MESSAGES.adminStandardUpdateSuccessF(I18N.MESSAGES.adminStandardLayoutGroup() + " '" + name + "'"));

                } else {
                    // Creation case.
                    N10N.infoNotif(I18N.CONSTANTS.adminPhaseCreationBox(),
                            I18N.MESSAGES.adminStandardCreationSuccessF(I18N.MESSAGES.adminStandardLayoutGroup() + " '" + name + "'"));
                }
                hideView();

                // Send an update event to reload necessary data.
                eventBus.fireEvent(new UpdateEvent(UpdateEvent.LAYOUT_GROUP_UPDATE, lg));
            }
