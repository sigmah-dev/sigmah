...
/**
 * UI component that provides a hierarchial list of navigation links. To use, you must implement
 * {@link org.sigmah.client.page.common.nav.Navigator}, which provides the data for
 * {@link com.extjs.gxt.ui.client.widget.treepanel.TreePanel}
 */
        ...
        tree.addListener(Events.OnClick, new Listener<TreePanelEvent<Link>>() {
            ...
            public void handleEvent(TreePanelEvent<Link> tpe) {
                if (tpe.getItem().getPageState() != null) {
                    eventBus.fireEvent(new NavigationEvent(NavigationHandler.NavigationRequested, tpe.getItem()
                        .getPageState(), null));
                }
            }
        });
