...
public class DashboardView extends ContentPanel implements DashboardPresenter.View {
	...
	private void buildNavLinks(final ContentPanel menuPanel) {
		...
		if (ProfileUtils.isGranted(authentication, GlobalPermissionEnum.EDIT_PROJECT)) {
			addNavLink(eventBus, menuPanel, I18N.CONSTANTS.importItem(), null, new Listener<ButtonEvent>() {

				@Override
				public void handleEvent(ButtonEvent be) {

					final ImportWindow importWindow = new ImportWindow(dispatcher, authentication, cache);
					importWindow.show();
				}
			});
		}
	...
	private Component buildProjectPanel() {
		...
		projectsListPanel = new ProjectsListPanel(dispatcher, authentication, eventBus,
		                ProjectsListPanel.RefreshMode.BOTH, ProjectsListPanel.LoadingMode.CHUNK);
