...
public class DbConfigPresenter implements Page {
    ...
    public void go(UserDatabaseDTO db) {
        ...
        if (db.isDesignAllowed()) {
            view.add(I18N.CONSTANTS.design(), I18N.CONSTANTS.designDescription(), "db-design.png", new DbPageState(
                DesignPanelActivityInfo.PAGE_ID, db.getId()));
        }
        if (db.isManageAllUsersAllowed()) {
            view.add(I18N.CONSTANTS.partner(), I18N.CONSTANTS.partnerEditorDescription(), "db-partners.png",
                new DbPageState(DbPartnerEditor.DatabasePartners, db.getId()));
        }
        if (db.isManageUsersAllowed()) {
            view.add(I18N.CONSTANTS.users(), I18N.CONSTANTS.userManagerDescription(), "db-users.png", new DbPageState(
                DbUserEditor.DatabaseUsers, db.getId()));
        }
