...
public class AdminOneModelPresenter {
	...
	private void addAllTabs() {
		for (int i = 0; i < MAIN_TABS.length; i++) {
			...
			tabItem.setId(String.valueOf(index));

			tabItem.addListener(Events.BeforeSelect, new Listener<BaseEvent>() {

				@Override
				public void handleEvent(BaseEvent be) {
					final TabItem item = AdminOneModelPresenter.this.view.getTabPanelParameters().getItemByItemId(
					                String.valueOf(index));

					if (!item.equals(currentTabItem)) {
						NavigationEvent event = new NavigationEvent(NavigationHandler.NavigationRequested, currentState
						                .deriveTo(currentState.getCurrentSection(), currentState.getModel(),
						                                MAIN_TABS[index], currentState.isProject()), null);
	...
	private void selectTab(String subModel, AdminPresenter.View adminView, Object model, boolean force,
		...
		if (index != -1) {
			...
			if (!item.equals(currentTabItem)) {
				...
				this.view.getTabPanelParameters().setSelection(
				                this.view.getTabPanelParameters().getItemByItemId(String.valueOf(index)));
	 ...
	 * @return {@link org.sigmah.client.page.admin.model.common.StatusChangeAction}
	 *         object
	...
	private StatusChangeAction isValidStatusChange(ProjectModelStatus currentStatus, ProjectModelStatus targetStatus) {
		...
		// model
		                ...
		                && !(targetStatus.equals(ProjectModelStatus.DRAFT) || targetStatus
		                                .equals(ProjectModelStatus.UNAVAILABLE))) {
			statusChangeAction.setValid(false);
	...
	/**
	 * Check if the change action of model's status is valid when the current
			...
			public void onSuccess(ModelCheckResult result) {
				...
				if (result == null || result.isUsed() == null) {// Row back
					...
					if (result.isUsed() && !targetStatus.equals(ProjectModelStatus.USED)) {
						...
						// I18N.CONSTANTS.unavailableUsedModelStatusChangeError(),
						// null);
					...
					// shift back to "Ready" model.
						...
						// MessageBox.alert(I18N.CONSTANTS.error(),
						// I18N.CONSTANTS.unavailableNotUsedModelStatusChangeError(),
						// null);
	...
	private void onOrgUnitModelStatusChanged() {
		...
		if (!view.getCurrentOrgUnitModel().getStatus().equals(ProjectModelStatus.UNAVAILABLE)) {
			StatusChangeAction statusChangeAction = isValidStatusChange(view.getCurrentOrgUnitModel().getStatus(),
		...
		if (!view.getCurrentOrgUnitModel().getStatus().equals(ProjectModelStatus.UNAVAILABLE)) {
			...
			if (!statusChangeAction.isValid()) {// Unvalid status changing
				...
				if (view.getCurrentProjectModel().getStatus().equals(ProjectModelStatus.DRAFT)) {
					...
					MessageBox.confirm(
					                I18N.MESSAGES.adminModelStatusChangeBox(),
					                ...
					                new Listener<MessageBoxEvent>() {
						                ...
						                public void handleEvent(MessageBoxEvent be) {
							                ...
							                if (Dialog.NO.equals(be.getButtonClicked().getItemId())) {
								                view.getStatusList().setSimpleValue(
								                                ProjectModelStatus.getName(view
								                                                .getCurrentOrgUnitModel().getStatus()));
							                }
						                }
					...
					Log.debug("Current status: "
					                + ProjectModelStatus.getName(view.getCurrentOrgUnitModel().getStatus())
					                + " target status: " + view.getStatusList().getValue().getValue());
					// Do nothing
				}
			}
	...
	private void onProjectModelStatusChanged() {
		...
		if (!view.getCurrentProjectModel().getStatus().equals(ProjectModelStatus.UNAVAILABLE)) {
			StatusChangeAction statusChangeAction = isValidStatusChange(view.getCurrentProjectModel().getStatus(),
			                ProjectModelStatus.getStatus(view.getStatusList().getValue().getValue()));
		                ...
		                new Listener<MessageBoxEvent>() {
			                ...
			                public void handleEvent(MessageBoxEvent be) {
				                if (Dialog.NO.equals(be.getButtonClicked().getItemId())) {
					                ...
					                cmdGetProjectsByModel.setProjectModelId(new Long((long) view
					                                .getCurrentProjectModel().getId()));

					                dispatcher.execute(
					                                cmdGetProjectsByModel,
					                                new MaskingAsyncMonitor(view.getTopPanel(), I18N.CONSTANTS
					                                                .loading()),
					                                new AsyncCallback<ProjectListResult>() {
						                                ...
						                                public void onSuccess(ProjectListResult result) {
							                                ...
							                                List<ProjectDTOLight> testProjects = result
							                                                .getListProjectsLightDTO();
							                                if (result != null && testProjects != null
							                                                && testProjects.size() > 0) {
								                                ...
								                                String waringMessage = I18N.MESSAGES
								                                                .DraftProjectModelChangeStatusDetails(
								                                                                testProjectNames,
								                                                                ProjectModelStatus
								                                                                                .getName(targetStatus));
								                                ...
								                                Listener<MessageBoxEvent> l = new Listener<MessageBoxEvent>() {
									                                ...
									                                public void handleEvent(MessageBoxEvent be) {
										                                ...
										                                if (be.getButtonClicked().getItemId()
										                                                .equals(Dialog.NO)) {
											                                view.getStatusList()
											                                                .setSimpleValue(ProjectModelStatus
											                                                                .getName(view.getCurrentProjectModel()
											                                                                                .getStatus()));
										                                }
								                                ...
								                                MessageBox.confirm(
								                                                I18N.CONSTANTS.projectChangeStatus(),
								                                                waringMessage, l);
		...
		if (view.isProject()) {
			...
			dispatcher.execute(new CreateEntity("ProjectModel", modelProperties), null,
			                new AsyncCallback<CreateResult>() {
				                ...
				                public void onFailure(Throwable caught) {
					                MessageBox.alert(I18N.CONSTANTS.adminProjectModelUpdateBox(), I18N.MESSAGES
					                                .adminStandardCreationFailure(I18N.CONSTANTS
					                                                .adminProjectModelStandard()
					                                                + " '"
				                ...
				                public void onSuccess(CreateResult result) {
					                if (result != null && result.getEntity() != null) {
						                ...
						                Notification.show(I18N.CONSTANTS.adminProjectModelUpdateBox(), I18N.MESSAGES
						                ...
						                eventBus.fireEvent(new NavigationEvent(NavigationHandler.NavigationRequested,
						                                currentState.deriveTo(currentState.getCurrentSection(),
						                                                currentState.getModel(), MAIN_TABS[tabIndex],
						                                                true), null));
			...
			dispatcher.execute(new CreateEntity("OrgUnitModel", modelProperties), null,
			                new AsyncCallback<CreateResult>() {
				                ...
				                public void onFailure(Throwable caught) {
					                MessageBox.alert(I18N.CONSTANTS.adminOrgUnitsModelCreationBox(), I18N.MESSAGES
					                                .adminStandardCreationFailure(I18N.CONSTANTS
					                                                .adminOrgUnitsModelStandard()
					                                                + " '"
					                                                + view.getNameField().getValue() + "'"), null);
				                }
				                ...
				                public void onSuccess(CreateResult result) {
					                if (result != null && result.getEntity() != null) {
						                ...
						                Notification.show(I18N.CONSTANTS.adminOrgUnitsModelCreationBox(), I18N.MESSAGES
						                                .adminStandardUpdateSuccess(I18N.CONSTANTS
						                                                .adminOrgUnitsModelStandard()
						                                                + " '"
						                                                + view.getNameField().getValue() + "'"));
