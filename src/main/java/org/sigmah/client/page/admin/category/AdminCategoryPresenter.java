...
public class AdminCategoryPresenter implements AdminModelSubPresenter {
    ...
    /**
     * Method to verify the deletion action. If the verification passes, try to delete a category, or show a alert
     * window.
     * 
     * @param selectedItems
     *            The selected categories
     ...
     * @author HUZHE (zhe.hu32@gmail.com)
     */
    private void deleteCategoryVerify(final List<CategoryTypeDTO> selectedItems,
            List<ProjectModelDTO> allProjectModelsList) {
        ...
        for (ProjectModelDTO projectModelDTO : allProjectModelsList) {
            ...
            for (FlexibleElementDTO e : allElements) {
                if (e.getElementType() == ElementTypeEnum.QUESTION) {
                    ...
                    if (questionElement.getCategoryTypeDTO() != null
                        && selectedItems.contains(questionElement.getCategoryTypeDTO())) {
                        deletionErrorList.add(new DeletionError(questionElement.getCategoryTypeDTO().getLabel(),
                            projectModelDTO.getName(), questionElement.getLabel()));
                    }
        ...
        if (deletionErrorList.size() > 0) {
            ...
            for (DeletionError error : deletionErrorList) {
                errorText =
                        errorText
                            + I18N.MESSAGES.categoryBeingUsed(error.getCategoryTypeName(), error.getProjectModelName(),
        ...
        // Else, try to delete
            ...
            dispatcher.execute(deactivate, null, new AsyncCallback<VoidResult>() {
                ...
                public void onSuccess(VoidResult result) {
                    ...
                    view.getCategoriesStore().commitChanges();
                    Notification.show(
                        I18N.CONSTANTS.adminCategoryTypeCreationBox(),
                        I18N.MESSAGES.adminStandardUpdateSuccessF(I18N.CONSTANTS.adminCategoryTypeStandard()
                            + " '"
    ...
    /**
     * Method to verify the deletion action. If the verification passes, try to delete a category element, or show a
     * alert window.
     * 
     * @param selectedItems
     ...
     * @author HUZHE (zhe.hu32@gmail.com)
     */
    protected void deleteCategoryElementVerify(final List<CategoryElementDTO> selectedItems,
            List<ProjectModelDTO> allProjectModelsList) {
        ...
        for (ProjectModelDTO projectModelDTO : allProjectModelsList) {
            ...
            for (FlexibleElementDTO e : allElements) {
                if (e.getElementType() == ElementTypeEnum.QUESTION) {
                    ...
                    if (questionElement.getCategoryTypeDTO() != null
                        && parentCategoryTypeDTO.getId() == questionElement.getCategoryTypeDTO().getId()) {
                        // Add a deletion error object
                        deletionErrorList.add(new DeletionError(parentCategoryTypeDTO.getLabel(), projectModelDTO
                            .getName(), questionElement.getLabel()));
                    }
    ...
    public static void refreshCategoryTypePanel(Dispatcher dispatcher, final View view) {
        dispatcher.execute(new GetCategories(), view.getReportModelsLoadingMonitor(),
