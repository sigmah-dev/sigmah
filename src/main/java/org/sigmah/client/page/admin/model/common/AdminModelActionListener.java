...
public class AdminModelActionListener implements ActionListener {
    ...
    public void onUIAction(String actionId) {
        /*
         * if (UIActions.delete.equals(actionId)) { view.confirmDeleteSelected(new ConfirmCallback() { public void
         * confirmed() { onDeleteConfirmed(view.getPrivacyGroupsSelection()); } }); }else
    ...
    private void onDeleteModel() {
        ...
        if (isProject) {// Delete draft project model
            ...
            if (getProjectModel() == null || !ProjectModelStatus.DRAFT.equals(getProjectModel().getStatus())) {
                MessageBox
                    .alert(I18N.CONSTANTS.deletionError(), I18N.CONSTANTS.deleteNotDraftProjectModelError(), null);
                return;
            }
            ...
            if (getOrgUnit() == null || !ProjectModelStatus.DRAFT.equals(getOrgUnit().getStatus())) {
                ...
                MessageBox
                    .alert(I18N.CONSTANTS.deletionError(), I18N.CONSTANTS.deleteNotDraftOrgUnitModelError(), null);
                return;
            }
    ...
    /**
     * RPC to execute the deletion of a project model Note: Only "Draft" is allowed to delete
     * 
     * @author HUZHE(zhe.hu32@gmail.com)
     */
        ...
        dispatcher.execute(deleteCommand, null, new AsyncCallback<VoidResult>() {
            ...
            public void onFailure(Throwable caught) {
                ...
                MessageBox.alert(I18N.CONSTANTS.deletionError(),
                    I18N.MESSAGES.entityDeleteEventError(getProjectModel().getName()), null);
            ...
            public void onSuccess(VoidResult result) {
                ...
                // Show notification.
                Notification.show(I18N.CONSTANTS.adminProjectModelDelete(),
                    I18N.CONSTANTS.adminProjectModelDeleteDetail());
            }
    ...
    /**
     * RPC to execute the deletion of a OrgUnit model Note: Only "Draft" is allowed to delete
     * 
     * @author HUZHE(zhe.hu32@gmail.com)
     */
    private void deleteOrgUnitModel() {
        ...
        dispatcher.execute(deleteCommand, null, new AsyncCallback<VoidResult>() {
            ...
            public void onFailure(Throwable caught) {
                ...
                MessageBox.alert(I18N.CONSTANTS.deletionError(),
                    I18N.MESSAGES.entityDeleteEventError(getOrgUnit().getName()), null);
            ...
            public void onSuccess(VoidResult result) {
                ...
                // Show notification.
                Notification.show(I18N.CONSTANTS.adminOrgUnitModelDelete(),
                    I18N.CONSTANTS.adminOrgUnitModelDeleteDetail());
            }
    ...
    /*
     * protected void onDeleteConfirmed(final List<PrivacyGroupDTO> selection) { List<Integer> ids = new
     * ArrayList<Integer>(); String names = ""; for(PrivacyGroupDTO s : selection){ ids.add(s.getId()); names =
     * s.getTitle() + ", " + names; } final String toDelete = names; final DeleteList delete = new
     * DeleteList(PrivacyGroup.class, ids); dispatcher.execute(delete, null, new AsyncCallback<VoidResult>() {
     * @Override public void onFailure(Throwable caught) { MessageBox.alert(I18N.CONSTANTS.error(),
     * I18N.MESSAGES.entityDeleteEventError(toDelete), null); }
     * @Override public void onSuccess(VoidResult result) { for(PrivacyGroupDTO model : selection){
        ...
        if (isProject) {
            ...
            final ProjectModelForm form = new ProjectModelForm(dispatcher, new AsyncCallback<CreateResult>() {
                ...
                public void onSuccess(CreateResult result) {
                    ...
                    Element addedRow =
                            ((AdminProjectModelsPresenter.View) view).getProjectModelGrid().getView().getRow(rowIndex);

                    // Second,set the selected model
                    ((AdminProjectModelsPresenter.View) view).getProjectModelGrid().getSelectionModel()
                        .setSelection(selectedModels);
            ...
            final OrgUnitModelForm form = new OrgUnitModelForm(dispatcher, new AsyncCallback<CreateResult>() {
                ...
                public void onSuccess(CreateResult result) {
                    ...
                    Element addedRow =
                            ((AdminOrgUnitModelsPresenter.View) view).getOrgUnitModelGrid().getView().getRow(rowIndex);

                    // Second,set the selected model
                    ((AdminOrgUnitModelsPresenter.View) view).getOrgUnitModelGrid().getSelectionModel()
                        .setSelection(selectedModels);
    ...
    private void onExportModel() {
        String parametres =
                "type="
                    + URL.encodeComponent("project-model")
                    + "&id="
        ...
        if (!isProject) {
            if (isReport) {
                parametres =
                        "type="
                            + URL.encodeComponent("project-report-model")
                            + "&id="
                            + URL.encodeComponent(String.valueOf(getModelId()));
            } else {
                if (isOrgUnit) {
                    parametres =
                            "type="
                                + URL.encodeComponent("org-unit-model")
                                + "&id="
                                + URL.encodeComponent(String.valueOf(getModelId()));
                } else {
                    parametres =
                            "type="
                                + URL.encodeComponent("category-type")
                                + "&id="
                                + URL.encodeComponent(String.valueOf(getModelId()));
                }
            }
        }
        com.google.gwt.user.client.Window.Location.replace(GWT.getModuleBaseURL() + "models?" + parametres);
    }
    ...
    private void onImportModel() {
        ...
        importPanel.addListener(Events.Submit, new Listener<FormEvent>() {
            ...
            public void handleEvent(FormEvent be) {
                ...
                if (result.indexOf("HTTP ERROR") != -1) {
                    MessageBox.alert(I18N.CONSTANTS.error(), I18N.CONSTANTS.adminProjectModelImportError(), null);
                }
                // Import succeed.
                else {
                    // Show notification
                    if (isProject)
                        Notification.show(I18N.CONSTANTS.adminProjectModelImport(),
                            I18N.CONSTANTS.adminProjectModelImportDetail());
                    else if (isOrgUnit)
                        Notification.show(I18N.CONSTANTS.adminOrgUnitsModelImport(),
                            I18N.CONSTANTS.adminOrgUnitsModelImportDetail());
                    else if (isReport)
                        Notification.show(I18N.CONSTANTS.adminReportModelImport(),
                            I18N.CONSTANTS.adminReportModelImportDetail());
                    else
                        Notification.show(I18N.CONSTANTS.adminCategoryImport(),
                            I18N.CONSTANTS.adminCategoryImportDetailt());
                }
        ...
        importDialog.getButtonById(Dialog.OK).addSelectionListener(new SelectionListener<ButtonEvent>() {
            ...
            public void componentSelected(ButtonEvent ce) {
                if (importPanel.isValid()) {
                    ...
                    if (isProject)
                        ...
                        MessageBox.alert(I18N.CONSTANTS.createFormIncomplete(),
                            I18N.MESSAGES.importFormIncompleteDetails(I18N.CONSTANTS.adminOrgUnitsModelStandard()),
                            null);
                    else if (isReport)
                        MessageBox.alert(I18N.CONSTANTS.createFormIncomplete(),
    ...
    private void onCopyModel() {
        if (isProject) {
            ...
            dispatcher.execute(copyProjectModelCommand, null, new AsyncCallback<ProjectModelDTOLight>() {

                @Override
                public void onFailure(Throwable arg0) {
                    MessageBox.alert(I18N.CONSTANTS.adminProjectModelCopy(),
                        I18N.CONSTANTS.adminProjectModelCopyError(), null);
                }

                @Override
                public void onSuccess(ProjectModelDTOLight result) {
                    if (result != null) {
                        ((AdminProjectModelsPresenter.View) view).getAdminModelsStore().add(result);
                        ((AdminProjectModelsPresenter.View) view).getAdminModelsStore().commitChanges();
                        // Show notification.
                        Notification.show(I18N.CONSTANTS.adminProjectModelCopy(),
                            I18N.CONSTANTS.adminProjectModelCopyDetail());
                        ...
                        Element addedRow =
                                ((AdminProjectModelsPresenter.View) view).getProjectModelGrid().getView()
                                    .getRow(rowIndex);
        ...
        if (isOrgUnit) {
            ...
            dispatcher.execute(copyOrgUnitModelCommand, null, new AsyncCallback<OrgUnitModelDTO>() {

                @Override
                public void onFailure(Throwable arg0) {
                    MessageBox.alert(I18N.CONSTANTS.adminOrgUnitsModelCopy(),
                        I18N.CONSTANTS.adminOrgUnitsModelCopyError(), null);
                }

                @Override
                public void onSuccess(OrgUnitModelDTO result) {
                    if (result != null) {
                        ((AdminOrgUnitModelsPresenter.View) view).getAdminModelsStore().add(result);
                        ((AdminOrgUnitModelsPresenter.View) view).getAdminModelsStore().commitChanges();
                        // Show notification.
                        Notification.show(I18N.CONSTANTS.adminOrgUnitsModelCopy(),
                        ...
                        Element addedRow =
                                ((AdminOrgUnitModelsPresenter.View) view).getOrgUnitModelGrid().getView()
                                    .getRow(rowIndex);
    ...
    private void refreshDataModelList() {
        if (isProject) {// refresh project models list
            AdminProjectModelsPresenter.refreshProjectModelsPanel(dispatcher, (AdminProjectModelsPresenter.View) view);
        } else {
            if (isOrgUnit) {// refresh org-unit models list
                AdminOrgUnitModelsPresenter.refreshOrgUnitModelsPanel(dispatcher,
                    (AdminOrgUnitModelsPresenter.View) view);
            } else {
                if (isReport) { // refresh reports model list
                    AdminReportModelPresenter.refreshReportModelsPanel(dispatcher,
                        (AdminReportModelPresenter.View) view);
