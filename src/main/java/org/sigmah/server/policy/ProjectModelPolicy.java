	...
	public Object create(User user, PropertyMap properties) {
		...
		if (projectModel != null) {
			if (projectModel.getId() != -1) {
				// properties can only contain actual changes between old
				// version and new one as verification has already been done
	...
	public void update(User user, Object entity, PropertyMap changes) {
		...
		if (model != null) {
			...
			if (phaseDTOToSave != null) {
				...
				else if (root != null && !root && phaseFound != null
				                && model.getRootPhase().getId() == phaseFound.getId())// Only
				                                                                      // if
				                                                                      // phase
				                                                                      // was
				                                                                      // root
