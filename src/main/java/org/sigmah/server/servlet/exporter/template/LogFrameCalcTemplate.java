...
public class LogFrameCalcTemplate implements ExportTemplate {
	...
	public LogFrameCalcTemplate(final LogFrameExportData data, final SpreadsheetDocument exDoc) throws Throwable {
		...
		cellRange = table.getCellRangeByPosition(1, rowIndex, data.getNumbOfCols(), rowIndex);
		cellRange.merge();
		...
		if (data.getEnableSpecificObjectivesGroups()) {
			...
			for (final LogFrameGroup soGroup : data.getSoMap().keySet()) {
				...
				if (!titleIsSet) {
					putTypeCell(rowIndex, data.getLocalizedVersion("logFrameSpecificObjectives"), data.getLocalizedVersion("logFrameSpecificObjectivesCode"));
					titleIsSet = true;
				}
		...
		if (data.getEnableExpectedResultsGroups()) {
			...
			for (final LogFrameGroup erGroup : data.getErMap().keySet()) {
				...
				// type (only once) SO,ER,A
				if (!titleIsSet) {
					putTypeCell(rowIndex, data.getLocalizedVersion("logFrameExceptedResults"), data.getLocalizedVersion("logFrameExceptedResultsCode"));
					titleIsSet = true;
				}
		...
		if (data.getEnableActivitiesGroups()) {
			...
			for (final LogFrameGroup aGroup : data.getAcMap().keySet()) {
				...
				// type (only once) SO,ER,A
				if (!titleIsSet) {
					putTypeCell(rowIndex, data.getLocalizedVersion("logFrameActivities"), data.getLocalizedVersion("logFrameActivitiesCode"));
					titleIsSet = true;
				}
		...
		if (data.getEnablePrerequisitesGroups()) {
			...
			for (final LogFrameGroup pGroup : data.getPrMap().keySet()) {
				...
				// type (only once) SO,ER,A
				if (!titleIsSet) {
					putTypeCell(rowIndex, data.getLocalizedVersion("logFramePrerequisites"), data.getLocalizedVersion("logFramePrerequisitesCode"));
					titleIsSet = true;
				}
	...
	private int putPrItems(int rowIndex, boolean skipFirst, List<Prerequisite> prList) {
		for (final Prerequisite p : prList) {
			...
			cellRange = table.getCellRangeByPosition(3, rowIndex, data.getNumbOfCols(), rowIndex);
			cellRange.merge();
		}
		return rowIndex;
	}

	private int putAcItems(int rowIndex, boolean skipFirst, List<LogFrameActivity> acList) throws Throwable {
	...
	private int putERItems(int rowIndex, boolean skipFirst, List<ExpectedResult> erList) throws Throwable {
	...
	private int putSOItems(int rowIndex, boolean skipFirst, List<SpecificObjective> soList) throws Throwable {
	...
	private void mergeCell(int startCol, int startRow, int endCol, int endRow) {
		cellRange = table.getCellRangeByPosition(startCol, startRow, endCol, endRow);
		cellRange.merge();
	}
	...
	private void putGroupCell(int rowIndex, String groupType, String code, String groupLabel) {
		...
		cellRange = table.getCellRangeByPosition(2, rowIndex, data.getNumbOfCols(), rowIndex);
		cellRange.merge();
	}
